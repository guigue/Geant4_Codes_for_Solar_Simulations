 ...
 : G4UserRunAction()
{
  ...
  theImportantEnergy =  100.0 * keV;  // Arbitrary
  theNumberOfTrials  =   100;  // Arbitrary
...
//analysisManager->CreateH1("Histogram", "Primary Protons Kinetic Energy", 100, 200, 1000*MeV);
...
void Solar_RunAction::BeginOfRunAction(const G4Run* aRun)
{
  // Open an output file
  //
  G4String fileName = "analysis/spectrums_tab.g4";
  G4AnalysisManager* analysisManager = G4AnalysisManager::Instance();
